...
export TRIAL=181212_score

#export READ_DIR=/work2/hacone/2018/human_centromeres/pacbio/Centro_Fasta_143Cells

export READ_DIR=/work2/hacone/2018/human_centromeres/pacbio/blast/ScoreMovies/
export REFNAME=ScoreMovies
...
prep_dbs() {
	zcat ${READ_DIR}/*.fasta.gz > ${REFNAME}.fasta
	makeblastdb -dbtype nucl -in ${REFNAME}.fasta -parse_seqids
}; export -f prep_dbs

align() {
	MON_NAME=$1
	samtools faidx ${MONOMER_DB} ${MON_NAME} > queries_${TRIAL}/${MON_NAME}.fa
	blastn -db ${REFNAME}.fasta -query queries_${TRIAL}/${MON_NAME}.fa -out blast_output_${TRIAL}/${MON_NAME} \
...
cut -f1 ${MONOMER_DB}.fai | head -n 10 | xargs -P 12 -I % bash -c "align %"
echo "done alignment"

cut -f1 ${MONOMER_DB}.fai | head -n 10 | xargs -P 12 -I % bash -c "split_by_movie %"
echo "done split"
