...
def draw_align(aln, bits, arrs, name = "align", pink = False):
    ...
    def draw_unit(r, i, x, y, flip = False, col = "grey"): # showing the unit # add 20 to y if necessarry
        ...
        if i > -1: 
            ...
            #if not flip:
            #    unit_a.add(dwg.line(
            #        start=(x + 2 + nvars, y), end=(x + 2 + nvars, y - 4),
            #        stroke_width=1, stroke="grey"))
            #else:
            #    unit_a.add(dwg.line(
            #        start=(x + 2 + nvars, y + 10), end=(x + 2 + nvars, y + 14),
            #        stroke_width=1, stroke="grey"))
    ...
    def draw_comp(i, j, x, y): # showing the alignment of two units # TODO add 10 to y when called
        ...
        if i > -1 and j > -1:
            ...
            for n, (e, f) in enumerate(zip(r[i,:], q[j,:])):
                if e * f == 0:
                    continue # no comparison at HOR variants!
                aln_ab.add(dwg.line(
                    start = (x + 2 + n, y + 1), end = (x + 2 + n, y + 9),
                    stroke = pair2col[(e, f)], stroke_width=1))
    ...
    xcoord = xoff + k
    for i in range(lj):
        draw_unit(q, i, xcoord * xskip, ycoord, col=t2col2[qa[i][2]])
        xcoord += 1
    ...
    # comp at y = 30 
    rs, qs = max(0, sbk), max(-sbk, 0) # aligned parts of reads
    lov = min(lj, -sbk + li) if sbk > 0 else min(li, sbk + lj) # length of overlap
    re, qe = rs + lov, qs + lov
    xcoord = xoff + rs
    for i in range(rs, re):
        draw_comp(i, qs - rs + i, xcoord * xskip, ycoord + 30)
        xcoord += 1

    # q at y = 40 at k
    xcoord = xoff + sbk
    for i in range(lj):
        draw_unit(q, i, xcoord * xskip, ycoord + 40, col=t2col2[qa[i][2]])
        xcoord += 1
...
if __name__ == '__main__':
    ...
    if args.action == "print-var":
        ...
        print(f"# Variants on HOR units of type: {hor_type} / {len(units)}")
        print_snvs(v, sort = "freq", n_tests = len(units), innum = True if args.innum else False)
